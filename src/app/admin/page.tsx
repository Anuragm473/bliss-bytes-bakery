      ...
      >
        ...
        <div className="rounded-3xl overflow-hidden border border-white/20 shadow-2xl bg-white">
          ...
          <div className="px-5 py-3 bg-white border-t border-pink-50 flex items-center gap-3">
            ...
            <span className="ml-auto text-[10px] text-[#C49BAA] font-semibold whitespace-nowrap">
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ring-1 ${m.bg} ${m.text} ${m.ring}`}
    >
...
function KPICard({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-3xl p-5 shadow-sm border border-white/70 bg-gradient-to-br ${from} ${to} hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200`}
    >
      <div className="absolute -top-6 -right-6 w-24 h-24 rounded-full bg-white/10" />
      <div className="absolute -bottom-8 -left-4 w-20 h-20 rounded-full bg-white/5" />
      <div className="relative">
        <span className="text-3xl leading-none">{icon}</span>
        <p className="mt-3 text-2xl font-black tracking-tight text-[#3D1A28]">
          {value}
        </p>
        <p className="text-[10px] font-bold text-[#8B5870] uppercase tracking-widest mt-0.5">
          {label}
        </p>
          ...
          return (
            <div key={step} className="flex-1 flex flex-col items-center">
              <div className="flex items-center w-full">
                <div
                  className={`relative w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0 transition-all duration-300 ring-2
                  ${
                    active
                      ? "ring-[#E8577A] bg-gradient-to-br from-[#E8577A] to-[#F7A072] shadow-lg scale-110"
                      : done
                ...
                {i < STATUS_FLOW.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 mx-1 rounded transition-colors duration-300 ${i < currentIdx ? "bg-emerald-300" : "bg-gray-200"}`}
                  />
                )}
              </div>
              <p
...
function AnalyticsPanel({ orders }: { orders: Order[] }) {
  ...
  const revenueWeek = orders
    .filter((o) => isThisWeek(o.createdAt) && o.status !== "cancelled")
    .reduce((s, o) => s + o.totalPrice, 0);
  const revenueMonth = orders
    .filter((o) => isThisMonth(o.createdAt) && o.status !== "cancelled")
    .reduce((s, o) => s + o.totalPrice, 0);
  ...
  return (
    <section className="bg-white/70 backdrop-blur-sm rounded-3xl border border-pink-50 shadow-sm p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-base font-black text-[#3D1A28] tracking-tight">
            Revenue Analytics
          </h2>
        ...
        <div className="flex flex-wrap gap-3">
          {[
            ...
            <div
              key={s.label}
              className="text-center px-4 py-2 rounded-2xl bg-gradient-to-br from-[#FDE8EE]/60 to-[#FDDDC8]/50 border border-pink-100"
            >
              <p className="text-[10px] text-[#C49BAA] font-bold uppercase tracking-widest">
                {s.icon} {s.label}
              </p>
              <p className="text-sm font-black text-[#3D1A28]">
                {fmtCurrency(s.value)}
              </p>
            </div>
          ))}
      ...
      <div className="h-44">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} barCategoryGap="30%">
            ...
            <XAxis
              dataKey="label"
              tick={{ fontSize: 11, fill: "#C49BAA", fontWeight: 600 }}
              axisLine={false}
              tickLine={false}
            />
            <YAxis
              tick={{ fontSize: 10, fill: "#C49BAA" }}
              axisLine={false}
              tickLine={false}
              tickFormatter={(v) => `₹${v}`}
              width={48}
            ...
            <Tooltip
              contentStyle={{
                ...
                fontWeight: 700,
                color: "#3D1A28",
                boxShadow: "0 4px 24px rgba(232,87,122,0.1)",
              }}
...
export default function AdminOrdersDashboard() {
    ...
    async (id: string, status: string) => {
      ...
      try {
        ...
        setOrders((arr) =>
          arr.map((o) =>
            o.id === id
              ? { ...o, status: (prev ?? "pending") as Order["status"] }
              : o,
          ),
        );
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (dateFilter === "today") r = r.filter((o) => isToday(o.createdAt));
      ...
      r = r.filter((o) => isThisWeek(o.createdAt));
  ...
  return (
    <>
      ...
      <div
        className="min-h-screen bg-gradient-to-br from-[#FFF0F3] via-[#FFF5EE] to-[#FFF0E8]"
        ...
        <header className="bg-white/85 backdrop-blur-md border-b border-pink-100/80 sticky top-0 z-50 shadow-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-black text-[#3D1A28] leading-tight tracking-tight">
                  Orders Dashboard
                </h1>
                <p className="text-[10px] text-[#C49BAA] font-semibold uppercase tracking-widest hidden sm:block">
                  Bliss Bites Bakery
                </p>
              </div>
            </div>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-7">
          ...
          <section className="sticky top-[65px] z-40 bg-white/85 backdrop-blur-md rounded-3xl border border-pink-100 shadow-sm px-5 py-4 space-y-3">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="relative flex-1 min-w-[200px]">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#C49BAA]"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2.5"
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                {(["all", "today", "week", "custom"] as DateFilter[]).map(
                  (df) => (
                    <button
                      key={df}
              ...
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className={`w-3.5 h-3.5 transition-transform ${sortNewest ? "" : "rotate-180"}`}
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2.5"
          ...
          {filteredOrders.length === 0 && (
            <div className="text-center py-24">
              ...
              <p className="text-[#8B5870] font-black text-lg">
                No orders found
              </p>
              <p className="text-sm text-[#C49BAA] mt-1.5">
                  ...
                  <thead>
                    <tr className="border-b border-pink-50 bg-gradient-to-r from-[#FFF0F3]/80 to-[#FFF5EE]/80">
                      {[
                        ...
                        <th
                          key={h}
                          className="px-4 py-3.5 text-left text-[10px] font-black text-[#8B5870] uppercase tracking-widest"
                        >
                          {h}
                        </th>
                      ))}
                        ...
                        <>
                          ...
                          >
                            <td className="px-4 py-3.5">
                              <code className="text-xs font-black text-[#E8577A] bg-pink-50 px-2 py-0.5 rounded-lg">
                                {order.orderNumber}
                              </code>
                            </td>
                            <td className="px-4 py-3.5">
                              <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#FDE8EE] to-[#FDDDC8] flex items-center justify-center flex-shrink-0 border border-pink-100">
                                  <span className="text-xs font-black text-[#E8577A]">
                                    {order.name.charAt(0).toUpperCase()}
                                  </span>
                                </div>
                                <div>
                                  <p className="text-sm font-bold text-[#3D1A28] leading-tight">
                                    {order.name}
                                  </p>
                                  <p className="text-[10px] text-[#C49BAA]">
                                    {order.phone}
                                  </p>
                                </div>
                              </div>
                            ...
                            <td className="px-4 py-3.5">
                              <p className="text-xs font-bold text-[#3D1A28]">
                                📅 {order.deliveryDate}
                              </p>
                              <p className="text-[10px] text-[#C49BAA]">
                                ⏰ {order.deliveryTime}
                            ...
                            <td className="px-4 py-3.5">
                              <span className="text-sm font-black text-[#E8577A]">
                                {fmtCurrency(order.totalPrice)}
                              </span>
                            </td>
                            <td className="px-4 py-3.5">
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-50 text-gray-600 ring-1 ring-gray-200 text-[10px] font-bold">
                                💵 COD
                              </span>
                            </td>
                            <td className="px-4 py-3.5">
                          ...
                          {isExpanded && (
                            <tr key={`exp-${order.id}`}>
                              <td
                                colSpan={8}
                                className="px-4 pb-5 pt-4 bg-gradient-to-r from-[#FFF5F7]/80 to-[#FFF5EE]/80 border-b border-pink-100 animate-fade-in"
                              >
                                <ExpandedDetails
                                  order={order}
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <h2 className="text-sm font-black text-[#3D1A28]">
                                {order.name}
                              </h2>
                              <code className="text-[10px] font-bold text-[#E8577A] bg-pink-50 px-1.5 py-0.5 rounded-lg">
                                {order.orderNumber}
                              </code>
                            </div>
                            <p className="text-xs text-[#C49BAA]">
                              {order.phone} · 📅 {order.deliveryDate}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3 flex-shrink-0">
                          ...
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className={`w-4 h-4 text-[#C49BAA] transition-transform duration-200 ${isExpanded ? "rotate-180" : ""}`}
                            viewBox="0 0 24 24"
                            fill="none"
                            stroke="currentColor"
                            strokeWidth="2.5"
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <InfoBox title="Delivery Address" icon="📍">
          <p className="text-sm text-[#3D1A28] font-medium">
            {order.address}, {order.area}
          </p>
          {order.landmark && (
            <p className="text-xs text-[#8B5870] mt-1">
              Near: {order.landmark}
            </p>
          )}
        ...
        <InfoBox title="Pricing Breakdown" icon="💰">
          <div className="space-y-1.5 text-sm">
            {[
              ...
              <div
                key={r.label}
                className="flex justify-between text-[#8B5870]"
              >
                <span>{r.label}</span>
          ...
          {order.items.map((item, i) => (
            ...
            >
              ...
              <div
                ...
                onClick={() =>
                  item.image && onImageClick(item.image, item.title)
                }
                title="Click to enlarge"
              >
                {item.image ? (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <p className="text-sm font-black text-[#3D1A28] leading-tight line-clamp-2">
                    {item.title}
                  </p>
      ...
      {order.user && (
        <div>
          <p className="text-[10px] font-black text-[#8B5870] uppercase tracking-widest mb-2">
            Account Info
          </p>
          <div className="flex items-center gap-3 px-4 py-3 bg-violet-50/60 rounded-xl border border-violet-100">
            ...
            <div>
              {order.user.name && (
                <p className="text-sm font-bold text-[#3D1A28]">
                  {order.user.name}
                </p>
              )}
              <p className="text-xs text-[#8B5870]">{order.user.phone}</p>
            </div>
            <span className="ml-auto text-[10px] bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full font-bold ring-1 ring-violet-200">
              Registered
            </span>
          </div>
        </div>
          ...
          {ALL_STATUSES.map((s) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <span className="ml-0.5 text-white/70 text-[10px]">✓</span>
                )}
              </button>
            );
          })}
          {isUpdating && (
            <span className="self-center text-xs text-[#C49BAA] animate-pulse font-semibold">
              Saving…
            </span>
          )}
        </div>
